...
echo "$($PSQL "SELECT distinct(name)  FROM games join teams on games.winner_id=teams.team_id or games.opponent_id=teams.team_id where year=2014 and round='Eighth-Final' order by name")"
...
echo "$($PSQL "SELECT name from games join teams on games.winner_id=teams.team_id where name like 'Co%' order by year ")"
